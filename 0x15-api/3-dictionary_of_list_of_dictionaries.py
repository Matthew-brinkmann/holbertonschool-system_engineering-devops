...
class UserControlObject:
    ...
    def userTodoTasksFromattedForJSON(self):
        """creates a list of each task in the TODO dictionary, removing
            obsolete data"""
        userName = self.currentUserInformation.get('username')
        userTaskList = []
        for task in self.currentUserTodoList:
            taskInformation = {}
            taskInformation["task"] = task.get('title')
            taskInformation["completed"] = task.get('completed')
            taskInformation["username"] = userName
            userTaskList.append(taskInformation)
        return (userTaskList)

    def createFormattedJSONObject(self, compileAll=False):
        """formats the dictionary for export"""
        formatedJSONObject = {}
        if compileAll is False:
            formatedJSONObject[self.currentSelectedUser] = \
                self.userTodoTasksFromattedForJSON()
        else:
            for user in self.allUserIdList:
                self.currentSelectedUser = user
                formatedJSONObject[user] = self.userTodoTasksFromattedForJSON()
        return (formatedJSONObject)

    def exportTodoListForAllUsersAsJSON(self, compileAll=False):
        """Prints user information in correct format"""
        if compileAll is False:
            with open("{}.json".format(self.currentSelectedUser),
                      'w') as jsonfile:
                json.dump(self.createFormattedJSONObject(compileAll), jsonfile)
        else:
            with open("todo_all_employees.json".
                      format(self.currentSelectedUser),
                      'w') as jsonfile:
                json.dump(self.createFormattedJSONObject(compileAll), jsonfile)


def getCurrentUserId():
    """returns current user ID, this is a placeholder
    function that will be replaced in real world,
    hense why it is trash."""
    try:
...
if __name__ == "__main__":
    ...
    userController.exportTodoListForAllUsersAsJSON(compileAll=True)
